...
// OSM bright layer
const osmbURL =
  "https://tiles.stadiamaps.com/tiles/alidade_smooth/{z}/{x}/{y}{r}.png?key=48ca86c8-eae0-4b72-9fbd-1898be6aeebc";
L.tileLayer(osmbURL, {
  maxZoom: 14,
  attribution:
...
// Set the bin
const getColorPopulation = (value) =>
  value > 60000
    ? "#b30000"
    : value > 30000
    ? "#e34a33"
...
const getColorPercentile = (value) =>
  value == null
    ? "#888888"
    : value > 85
    ? "#b30000"
      ...
      currentLayer = L.geoJSON(data, {
        ...
        onEachFeature: (feature, layer) => {
          ...
          const salePrice = feature.properties.median_sale_price
            ? `$${(Number(feature.properties.median_sale_price) / 1000).toFixed(
                0
              )}k`
            : "N/A";
          const percentile = feature.properties.percentile
            ? feature.properties.percentile.toFixed(2)
            : "N/A";
...
function createLegend(map, getColorFn, grades) {
  ...
  legend.onAdd = function () {
    ...
    for (let i = 0; i < grades.length; i++) {
      labels.push(
        `<i style="background:${getColorFn(grades[i] + 1)}"></i> ${grades[i]}${
